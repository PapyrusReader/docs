...
This section outlines the user interface design principles, layout
patterns, and key screen specifications for the Papyrus cross-platform
book management system. The design follows Material 3 guidelines while
maintaining consistency across all supported platforms, with special
considerations for e-ink devices.
...
- Unified experience across Android, iOS, Web, Desktop, and E-ink
  platforms
...
- **High contrast**: Pure black on white (or inverse) for maximum
  readability
- **No animations**: Static UI transitions to avoid ghosting artifacts
- **Reduced refresh**: Minimize screen updates to improve battery life
  and reduce flicker
- **Larger touch targets**: Accommodate slower touch response on e-ink
  screens
- **Hardware button support**: Navigation via physical buttons when
  available
...
- Swipe gestures for quick actions (mark as read, favorite) - disabled
  on e-ink
...
- **Side panels:** Table of contents, annotations, notes, bookmarks
  (collapsible)
...
- **Series information:** Series name, book number, other books in
  series
...
- **Search bar:** Text input with voice search option (voice disabled on
  e-ink)
...
- **Detailed statistics:** Books by genre, reading speed, completion
  rates
...
E-ink devices require special UI considerations due to their unique
display characteristics:

**Display optimizations:**

- **No animations:** All transitions are instant cuts, no fades or
  slides
- **Minimal refresh:** UI updates batched to reduce screen flicker
- **Full page refresh:** Optional periodic full refresh to clear
  ghosting
...
- **Hardware buttons:** Page turn buttons mapped to forward/back
  navigation
- **Larger touch targets:** Minimum 56px touch areas (vs 44px on other
  platforms)
...
- **Contrast adjustment:** Fine-tune text darkness for different e-ink
  panels
- **Font optimization:** Use fonts optimized for e-ink (e.g., Bookerly,
  Literata)
...
The application uses Material 3 design system with custom color schemes
optimized for reading applications.
...
+================+===================+=============+===================+
| Primary        | Purple            | #5654A8     | Main brand color  |
|                |                   |             | for buttons,      |
|                |                   |             | links, key UI     |
|                |                   |             | elements          |
+----------------+-------------------+-------------+-------------------+
| Primary        | Light Purple      | #E2DFFF     | Backgrounds for   |
| Container      |                   |             | primary actions   |
+----------------+-------------------+-------------+-------------------+
| Secondary      | Dark Purple-Gray  | #5D5C71     | Secondary buttons |
|                |                   |             | and text          |
+----------------+-------------------+-------------+-------------------+
| Secondary      | Light Gray-Purple | #E3E0F9     | Secondary         |
| Container      |                   |             | backgrounds       |
+----------------+-------------------+-------------+-------------------+
| Surface        | Pure White        | #FFFBFF     | Main content      |
|                |                   |             | backgrounds       |
+----------------+-------------------+-------------+-------------------+
| Surface        | Light Gray        | #E4E1EC     | Card and          |
| Container      |                   |             | container         |
|                |                   |             | backgrounds       |
+----------------+-------------------+-------------+-------------------+
...
|                |                   |             | color             |
+----------------+-------------------+-------------+-------------------+
| On Surface     | Medium Gray       | #47464F     | Secondary text    |
| Variant        |                   |             | color             |
+----------------+-------------------+-------------+-------------------+
| Outline        | Medium Gray       | #787680     | Borders and       |
|                |                   |             | dividers          |
+----------------+-------------------+-------------+-------------------+
...
| Role           | Color             | Hex         | Usage             |
+================+===================+=============+===================+
| Primary        | Light Purple      | #C3C0FF     | Adjusted for dark |
|                |                   |             | backgrounds       |
+----------------+-------------------+-------------+-------------------+
| Primary        | Dark Purple       | #3E3C8F     | Primary action    |
| Container      |                   |             | backgrounds       |
+----------------+-------------------+-------------+-------------------+
| Secondary      | Light Purple-Gray | #C7C4DD     | Secondary         |
|                |                   |             | elements          |
+----------------+-------------------+-------------+-------------------+
| Secondary      | Dark Gray         | #464559     | Secondary         |
| Container      |                   |             | backgrounds       |
+----------------+-------------------+-------------+-------------------+
| Surface        | Very Dark Gray    | #1C1B1F     | Main content      |
|                |                   |             | backgrounds       |
+----------------+-------------------+-------------+-------------------+
| Surface        | Medium Dark Gray  | #47464F     | Card backgrounds  |
| Container      |                   |             |                   |
+----------------+-------------------+-------------+-------------------+
| On Surface     | Light Gray        | #E5E1E6     | Primary text      |
|                |                   |             | color             |
+----------------+-------------------+-------------+-------------------+
| On Surface     | Medium Light Gray | #C8C5D0     | Secondary text    |
| Variant        |                   |             | color             |
+----------------+-------------------+-------------+-------------------+
| Outline        | Gray              | #928F9A     | Borders and       |
|                |                   |             | dividers          |
+----------------+-------------------+-------------+-------------------+
...
- **Error colors:** Red tones (#BA1A1A light, #FFB4AB dark) for
  validation and warnings
- **Tertiary colors:** Pink-purple accent (#7A5368 light, #EAB9D2 dark)
  for special highlights
- **Shadow and scrim:** Pure black (#000000) for depth and overlays
- **Inverse colors:** Automatically calculated contrasting colors for
  accessibility

Dynamic theming features
~~~~~~~~~~~~~~~~~~~~~~~~

- **System integration:** Automatic theme switching based on system
  preferences
- **Material You support:** Dynamic color generation from wallpaper
  (Android 12+)
...
- **Custom accent colors:** User-customizable accent colors while
  maintaining accessibility
- **E-ink auto-detection:** Automatic switch to e-ink theme on
  compatible devices
...
+=====================+===========+=============+=====================+
| Mobile              | 0-599px   | Single      | Bottom tabs         |
|                     |           | column      |                     |
+---------------------+-----------+-------------+---------------------+
| Tablet              | 600-839px | Adaptive    | Side navigation     |
|                     |           | grid        | option              |
+---------------------+-----------+-------------+---------------------+
| Desktop Small       | 8         | Sidebar +   | Sidebar navigation  |
|                     | 40-1199px | m           |                     |
|                     |           | ulti-column |                     |
+---------------------+-----------+-------------+---------------------+
| Desktop Large       | 1200px+   | Multiple    | Sidebar navigation  |
|                     |           | panels      |                     |
+---------------------+-----------+-------------+---------------------+
| E-ink               | Any       | Simplified  | Tab bar or hardware |
|                     |           | single      | buttons             |
|                     |           | column      |                     |
+---------------------+-----------+-------------+---------------------+
...
- **High contrast modes:** Enhanced contrast ratios for better
  visibility
- **Font scaling:** Support for system font size preferences (up to
  200%)
...
- **Hardware button support:** Physical button navigation on supported
  devices
...
- **Confirmation dialogs:** Explicit confirmation for destructive
  actions

E-ink accessibility considerations
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

- **Slow refresh accommodation:** Extra time for users to read before
  screen updates
- **Clear state indication:** Obvious visual feedback for
  selected/active states
- **Reduced cognitive load:** Simplified screens with essential
  information only
- **Consistent layout:** Predictable element positioning across screens
- **Text-first design:** Information conveyed through text, not color or
  icons alone
...
This user interface specification provides a comprehensive framework for
developing a modern, accessible, and user-friendly book management
application that works seamlessly across all target platforms—including
e-ink devices—while maintaining the clean, Material 3-inspired design
aesthetic.
